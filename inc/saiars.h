...
typedef enum _sai_ars_quality_map_attr_t
{
    ...
    /**
     * @brief Device identifier list for each port in port list attribute
     *
     * @type sai_u32_list_t
     * @flags CREATE_AND_SET
     * @default empty
     */
    SAI_ARS_QUALITY_MAP_ATTR_DEVICE_ID_LIST,

    /**
     * @brief Quality adjustment value for corresponding port in the port list attribute
     * This value is directly programmed in the HW if firmware is not running.
     *
    ...
    /**
     * @brief L1 Next hop member of remote next hop group type
     *
     * @type sai_object_id_t
     * @flags CREATE_AND_SET
